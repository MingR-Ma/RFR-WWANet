...
class WWA(nn.Module):
    def __init__(self, dim, factor=0.25):
        ...
        self.mlp1 = nn.Sequential(
            nn.Linear(dim, int(dim * self.factor)),
            nn.ReLU(),
            nn.Linear(int(dim * self.factor), dim)
        )
        self.sigmoid = nn.Sigmoid()

        self.mlp2 = nn.Sequential(
            Rearrange('B L C -> L C B'),
            nn.Linear(self.N, int(self.N * self.factor)),
            nn.ReLU(),
            nn.Linear(int(self.N * self.factor), self.N),
            Rearrange('L C B -> B L C')
        )
    ...
    def forward(self, x):
        ...
        y2 = torch.mean(attn_window, dim=1, keepdim=True)
        y2 = self.mlp2(y2)
        y2 = self.sigmoid(y2)
        y2 = y2.expand_as(x)
        attn_window = y2 * x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.W = None
        self.T = None
        self.attn4attn = WWA(self.dim)
        if dim == 768:
            self.attn4attn = nn.Identity()
    ...
    def forward(self, x, mask_matrix):
        ...
        x_windows = self.attn4attn(x_windows)
...
class SwinTransformerHigh(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.head_up1 = PatchExpanding(embed_dim, scale_factor=4)

        self.head_up2 = PatchExpanding(embed_dim // 2, scale_factor=4)

        self.merging1 = PatchMerging(embed_dim // 2, reduce_factor=pat_merg_rf)

        self.head_norm1 = norm_layer(embed_dim // 2)
    ...
    def forward(self, x):
        ...
        x = self.head_st(x, Wh, Ww, Wt)
        out = x[4]
        out = out.view(-1, Wh, Ww, Wt, self.embed_dim).permute(0, 4, 1, 2, 3).contiguous()
        head_out1 = self.head_up1(out)
        head_out2 = self.head_up2(head_out1)

        out = rearrange(head_out1, 'B C D H W -> B (D H W) C')
        x = self.merging1(out, Wh * 2, Ww * 2, Wt * 2)

        outs.append(head_out2)
        outs.append(head_out1)
        Wh, Ww, Wt = Wh, Ww, Wt
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinTransformerHigh, self).train(mode)
        self._freeze_stages()
